...
libusb_context *LibCorsairRGB::ctxt = NULL;
struct libusb_device_handle *LibCorsairRGB::device_handle;
unsigned char LibCorsairRGB::verbose_level = 0;
char LibCorsairRGB::red_val[144];
char LibCorsairRGB::grn_val[144];
char LibCorsairRGB::blu_val[144];

void LibCorsairRGB::setVerbosityLevel(unsigned char level){
  verbose_level = level;
}

// Initialise the library by finding and attaching to the keyboard
int LibCorsairRGB::initialise(void){
  int status = 0;
...
void LibCorsairRGB::deinitialise(void){
...
void LibCorsairRGB::setKey(unsigned char x, unsigned char y, unsigned char r, unsigned char g, unsigned char b){
  k70_iso_keymap key = k70_iso_matrix[y][x];
  LibCorsairRGB::setKey(key, r, g, b);
}

// Set the RGB value of a key
void LibCorsairRGB::setKey(k70_iso_keymap key, unsigned char r, unsigned char g, unsigned char b){
  if (key == 0xFF) return;
...
void LibCorsairRGB::flushLightBuffer(void) {
